...
public class CreateGraviGen implements Listener {
    @EventHandler
    public void onPlayerInteract(PlayerInteractEvent event) {
        if (event.getAction() == Action.RIGHT_CLICK_BLOCK) {
            ItemStack item = event.getItem();
            if (item != null && item.getType() == Material.PAPER
                    && item.getItemMeta().getLore().equals(Arrays.asList("Это мощное устройство, изменяющее силу притяжения вокруг себя. Требует N-ое количество энергии."))
                    && item.getItemMeta().getDisplayName().equals("Генератор гравитации")) {
